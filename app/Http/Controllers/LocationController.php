...
class LocationController extends Controller
{
    ...
    //             'locations.slug',
    //             'locations.featured_image',
    //             'locations.per_night_charges',
    ...
    public function checkAvailability(Request $request)
{
    ...
    if ($dateIn && $dateOut) {
        $locations = $locations->where(function ($query) use ($dateIn, $dateOut) {
            $query->whereBetween('reservations.date_in', [$dateIn, $dateOut])
                  ->orWhereBetween('reservations.date_out', [$dateIn, $dateOut])
                  ->orWhere(function ($query) use ($dateIn, $dateOut) {
                      $query->where('reservations.date_in', '<=', $dateIn)
                            ->where('reservations.date_out', '>=', $dateOut);
                  });
        });
    }
